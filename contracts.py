...
def extract_cells(image: np.ndarray) -> list[np.ndarray]:
    ...
    list[np.ndarray]
        ...
        Each array:  shape (28, 28, 1), dtype float32, values in [0.0, 255.0]
        Note:        values are NOT pre-normalised — the model's first layer
                     (Rescaling 1/255) handles that at inference time.
    ...
    Steps to implement (suggested):
      ...
      5. For each cell: crop with 4px margin, resize to 28x28,
...
def predict_grid(cells: list[np.ndarray], model) -> list[list[int]]:
    ...
    cells : list[np.ndarray]
        ...
        Values in [0.0, 255.0] — the model normalises internally.
    ...
    list[list[int]]
        ...
        1-9 = detected digit.
        All values must be Python int (not numpy.int64).
...
#   Input shape:  (None, 28, 28, 1)  — batch of grayscale cell images, raw [0,255]
...
#   predictions = model.predict(batch)  # batch shape: (81, 28, 28, 1), values [0,255]
#
...
def _run_tests():
    ...
    try:
        ...
        try:
            ...
            print("  PASS — returns 81 x (28,28,1) float32 arrays")
            passed += 1
        except ValueError:
