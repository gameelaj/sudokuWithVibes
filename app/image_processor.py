...
#   6. Normalize to float32 [0,1] and reshape to (28,28,1)
#
...
def _extract_cells_from_warp(gray_warp: np.ndarray) -> list:
    """
    Divide a WARP_SIZE×WARP_SIZE grayscale image into 81 normalized cell arrays.

    Each cell is pre-processed:
      - Cropped with a small inner margin to reduce grid-line bleed
      - Resized to CELL_SIZE×CELL_SIZE
      - Normalized to float32 [0, 1]
      - Reshaped to (CELL_SIZE, CELL_SIZE, 1) — channel-last for Keras

    Returns a flat list of 81 np.ndarray, row-major (r*9 + c).
    """
    cells = []
    margin = 4  # pixels to crop from each edge of a cell to trim grid lines
    ...
    for r in range(9):
        for c in range(9):
            ...
            # Normalize [0,255] → [0.0, 1.0] and add channel dim
            normalized = resized.astype(np.float32) / 255.0
            normalized = normalized.reshape(CELL_SIZE, CELL_SIZE, 1)
...
def extract_cells(image: np.ndarray) -> list:
    ...
    list[np.ndarray]
        ...
        Each: shape (28, 28, 1), dtype float32, values in [0.0, 1.0].
