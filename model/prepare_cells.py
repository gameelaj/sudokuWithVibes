...
OUTPUT_ROOT = os.path.join(DATA_ROOT, "cells")   # xtracted cells go here
...
def order_points(pts: np.ndarray) -> np.ndarray:#reordering the corners to a standard format
...
def perspective_warp(image: np.ndarray, keypoints: list) -> np.ndarray:
    ...
    return gray #we get a warped(trasnformed) grayscale image


def extract_cells_from_warp(gray_warp: np.ndarray) -> list:
    cells = []
    for r in range(9):
        for c in range(9): #Divide a WARP_SIZE x WARP_SIZE grayscale image into 81 cell crops.
            y1 = r * CELL_PX + MARGIN
            y2 = (r + 1) * CELL_PX - MARGIN #Each crop is trimmed by MARGIN pixels on all four edges
            x1 = c * CELL_PX + MARGIN
    ...
    return cells #Returns a flat list of 81 uint8 arrays, row-major (index = r*9 + c).


def derive_class(cell_label: list) -> int:
    if cell_label[0] == 1:
        for digit in range(1, 10): #labeling cells into respective 0-9 classes
            if cell_label[digit] == 1: #using info from the metadata file
                return digit
...
def load_image_robust(path: str):
    ...
    try:
        ...
        return bgr   #Returns a BGR numpy array, or None if loading fails.
...
def main(): #main function to drive the file
